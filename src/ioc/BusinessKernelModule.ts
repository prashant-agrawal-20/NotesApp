...
export function BusinessKernelModule(kernel: Container) {
    return new ContainerModule((bind: interfaces.Bind) => {
        ...
        bind<LoggingOptions>(TYPES.LoggingOptions).toDynamicValue((context: interfaces.Context) => {
            const appConfig = context.container.get<any>(TYPES.Configuration).getConfiguration()
            ...
            if (options.logToFile && !options.logDir) {
                throw new Error("Logging Directory not found!, Please mention loggingDir in the conf")
            }
            return options
        })
