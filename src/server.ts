...
export class APIServer extends InversifyExpressServer {
    ...
    private handlerFactoryFunc(controllerName: any, key: string): express.RequestHandler {
        const that = this
        return function ApiRequestHandler(req: express.Request, res: express.Response, next: express.NextFunction) {
            let _value: any
            if (res.headersSent) {
            ...
            Promise.resolve(that.execute(controllerName, key, req, res, next))
                ...
                that.logger.error("Error sending api response: " + util.inspect(_value, false, 10, false))
                next(error)
            })
        }
    }
