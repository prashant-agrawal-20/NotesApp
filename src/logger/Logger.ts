...
export class Logger implements ILogger {
    ...
    private getWinstonTransportsConfig(winstonLoggingLevel: WinstonLoggingLevel): any {
        const transports = []
        const logToFile: boolean = !!(this.loggingOptions.logToFile && this.loggingOptions.logDir)
        if (logToFile) {
            transports.push(new DailyRotateFile({
                silent: false,
                filename: path.join(this.loggingOptions.logDir || "logs", winstonLoggingLevel + "-%DATE%.log"),
    ...
    constructor(
        ...
        if (!_.isNil(process.env.LOG_LEVEL) && LogLevels.includes(process.env.LOG_LEVEL)) {
            this.logLevel = process.env.LOG_LEVEL as unknown as LogLevel
        } else {
            this.logLevel = LogLevel.INFO
        }
        if (this.loggingOptions.logDir && !fs.existsSync(this.loggingOptions.logDir)) {
        ...
        this.infoLogger = createLogger({
            ...
            exitOnError: false,
            format: format.combine(format.splat(), format.simple(), format.timestamp({format: "YYYY-MM-DD HH:mm:ss"}), logFormat)
        })
    ...
    jsonifyMeta(msg: string, meta: any[]) {
        ...
        return msg + " meta: " + util.inspect(meta, { depth: 5, compact: true, breakLength: Infinity })
    }
