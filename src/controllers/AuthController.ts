...
export function authControllerFactory(kernel: Container) {
    ...
    class AuthController {
        ...
        @httpPost("/signup")
        async emailSignup(req: express.Request, res: express.Response): Promise<{success: boolean}> {
            return this.authBusiness.registerUser(req.body)
        }

        @httpPost("/login")
        async verifyLogin(req: express.Request, res: express.Response): Promise<ILoginResponse> {
